...
$app_role_name = "User"
####################################################################################
Write-Output "`n This script conducts the following: `n - The creation of a group that doesnt exist and updates to existing groups `n - Adds users to existing/new groups `n - Syns groups to AAD `n - Marks to be used in AWS SSO"
...
$To = @($EmailAddress)
Write-Output "`n When entering group names, enter each (one-per-line) in the following format BSAM_AWS_<placeholder>_<role>"
Write-Output "`n When entering usernames, enter each (one-per-line) in the following UPN format (example: user1@bfusa.com)"
#Write-Output "`n To stop entering groups and users, type the word end"
